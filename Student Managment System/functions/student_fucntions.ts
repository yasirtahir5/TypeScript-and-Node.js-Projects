...
let Std_columnName:string[]=["id","firstName", "lastName", "age", "grade","password","email"]
let std_options:string[]=["Remove Record","Modify your Record"]
...
async function Std_remove_record(){
 ...
 async function get_Student_Porgamm_Input(programm:string[]){
   let question = inquirer.prompt([{
    ...
    async function get_Student_Semester_Input(programm:number[]){
      let question = inquirer.prompt([{
      ...
      let student_log_in_verification =await inquirer.prompt([{
       ...
       async function ForgetPAssword(){
        let ForgetPAssword =await inquirer.prompt([{
         ...
         async function login(x:any){
          let verifyStudent_details=await verifyStudent()
          let singleStudents:any = await x.getStudentPasswordById(verifyStudent_details.IVerify_id,"id");
